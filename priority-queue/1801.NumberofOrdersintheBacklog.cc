...
class Solution {
	public:
		class Buy
		{
			public:
				int price,amount;
				Buy(int _price,int _amount):price(_price),amount(_amount){}
		...
		class Sell
		{
			public:
				int price,amount;
				Sell(int _price,int _amount):price(_price),amount(_amount){}
		...
		int getNumberOfBacklogOrders(vector<vector<int>>& orders) {
			...
			for(int i=0;i<n;i++)
			{
				...
				if(type==0)
				{
					while(amount>0&&!s.empty())
					{
						...
						if(mn.amount>amount)
						{
							mn.amount-=amount;
							amount=0;
							s.push(Sell(mn.price,mn.amount));
						}
						else
							amount-=mn.amount;
					}
				...
				else
				{
					...
					{
						Buy mx=b.top();
						if(mx.price<price) break;
						b.pop();
						if(mx.amount>amount)
						{
							mx.amount-=amount;
							amount=0;
							b.push(Buy(mx.price,mx.amount));
						}
						else
							amount-=mx.amount;
					}
			...
			while(!s.empty())
			{
				res=(res+s.top().amount)%mod;
				s.pop();
			}
			while(!b.empty())
			{
				res=(res+b.top().amount)%mod;
				b.pop();
			}
...
int main() 
{
	...
	cout<<s.getNumberOfBacklogOrders(v)<<endl;
	v.clear();v={{7,1000000000,1},{15,3,0},{5,999999995,0},{5,1,1}};
	cout<<s.getNumberOfBacklogOrders(v)<<endl;
	return 0;
}
