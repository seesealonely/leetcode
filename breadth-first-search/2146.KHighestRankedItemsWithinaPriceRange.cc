...
class Solution {
	public:
		vector<vector<int>> highestRankedKItems(vector<vector<int>>& grid, vector<int>& pricing, vector<int>& start, int k) {
			...
			sort(res.begin(), res.end(), [&](const vector<int> &p, const vector<int> &q) {
					int px = p[0], py = p[1], qx = q[0], qy = q[1];
					if (dis[px][py]!= dis[qx][qy])
					return dis[px][py] < dis[qx][qy];
...
{
	Solution s;
	vector<vector<int> > g={{1,2,0,1},{1,3,0,1},{0,2,5,1}};
	vector<int> p={2,5},st={0,0};
	show(s.highestRankedKItems(g,p,st,3));
	return 0;
}
