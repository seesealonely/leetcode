...
class Solution {
	public:
		...
		int getMax(vector<int> &arr)
		{
			...
			int res=1,pre=arr[0]-1;
			for(int i=1;i<arr.size();i++)
				if(arr[i]!=arr[i-1]+1)
				{
					res=max(res,arr[i-1]+1-pre);
					pre=arr[i]-1;
				}
			return max(res,arr.back()+1-pre);
		}
};
...
int main() 
{
	...
	cout<<s.maximizeSquareHoleArea(2,1,hBars,vBars)<<endl;
	hBars.clear();hBars={2};
	cout<<s.maximizeSquareHoleArea(1,1,hBars,vBars)<<endl;
	hBars.clear();vBars.clear();
	hBars={2,3},vBars={2,3,4};;
	cout<<s.maximizeSquareHoleArea(2,3,hBars,vBars)<<endl;
	return 0;
}
